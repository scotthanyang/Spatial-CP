...
(Xs_train, Ys_train, s_train, eps_train,
 ...
 X_grid, Y_grid, s_grid, eps_grid) = load_or_generate_data(
    sce=sce, n_data=n_data, grid_size=grid_size
)
...
qrf = RandomForestQuantileRegressor(n_estimators=50, max_depth=10, q=[0.05, 0.95],n_jobs=-1)
...
nn1.fit(Xc_cal)
Xc_test = np.concatenate([s_test,Xs_test.reshape((-1,1))],axis=1)
distances, indices_test = nn1.kneighbors(Xc_test)
feature_test = res_cal[indices_test[:,:-1]]
...
# residual plot
imshow_with_fixed_cbar(np.abs(res_grid).reshape((grid_size+1,-1)), title='true residuals')

# LSCP
res_grid_5,  res_grid_95 = qrf.predict(feature_grid)
len_grid_lscp = (res_grid_95-res_grid_5).reshape((grid_size+1,-1))
imshow_with_fixed_cbar(len_grid_lscp, title="LSCP")

# EnbPI
qg5, qg95 = np.quantile(feature_grid, q=[0.05,0.95],axis=1)
len_grid_enbpi = (qg95-qg5).reshape((grid_size+1,-1))
imshow_with_fixed_cbar(len_grid_enbpi, title="EnbPI")

# GSCP
q5, q95 = np.quantile(res_cal, [0.05, 0.95])
imshow_with_fixed_cbar(np.full_like(len_grid_lscp, q95 - q5), title="GSCP")

# SLSCP
weights = np.exp(-distances[:,:-1]**2/(2*bd**2))
qs5, qs95 = weighted_quantile(feature_grid, [0.05, 0.95], weights=weights)
imshow_with_fixed_cbar((qs95 - qs5).reshape((grid_size+1, -1)), title="SLSCP")

# LCP
weights = np.exp(-distances1[:,:-1]**2/(2*bd1**2))
ql5, ql95 = weighted_quantile(feature_grid1, [0.05, 0.95], weights=weights)
imshow_with_fixed_cbar((ql95 - ql5).reshape((grid_size+1, -1)), title="LCP")
